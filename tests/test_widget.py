...
def wrong_account_card(wrong_account_card):
...
def test_get_date_standard(sample_iso_date):
...
@pytest.mark.parametrize("invalid_input", [
    "",                # Пустая строка
    "   ",             # Строка из пробелов
    "not-a-date",      # Просто текст
    "2024/03/11",      # Неверный разделитель (если ожидается только ISO)
    "31.12.2024",      # Формат уже изменен (повторный вход)
    "2024-13-01T00:00" # Несуществующий месяц (13-й)
])
